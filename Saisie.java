...
public class Saisie extends JApplet {
	...
	class CButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent ae) {
			...
			String URL;
			try{
				Carnet carnet =(Carnet) Naming.lookup( "//"+InetAddress.getLocalHost().getHostName()+":4000/"+c);
				RFiche rfiche = (RFiche) carnet.Consulter(n, false);
				email.setText(rfiche.getEmail());
			}catch (Exception ef){
				ef.printStackTrace();
			}	
		}
	...
	class AButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent ae) {
			...
			try {
				Carnet carnet = (Carnet) Naming.lookup( "//"+InetAddress.getLocalHost().getHostName()+":4000/"+c);
				carnet.Ajouter(new SFicheImpl(n, email.getText()));
			} catch (Exception exc) {
				exc.printStackTrace();
			}
		}
